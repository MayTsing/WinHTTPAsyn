...
Abstract:
    ...
    注： 这个单元内容从 Delphi 10.3.2 目录中获取的，内容有些调整
...
const
  ...
  {$EXTERNALSYM WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY}
...
{ probably an invalid (non-code) address was supplied for the callback }
  ...
  WINHTTP_INVALID_STATUS_CALLBACK = -1; // : TWinHttpStatusCallback = Pointer(-1);
...
type
  TWinHTTPBinding = packed record
    ...
    CheckPlatform: function():BOOL; stdcall;
    /// Initializes an application's use of the WinHTTP functions.
    Open: function(pszAgentW: LPCWSTR; dwAccessType: DWORD; pszProxyW: LPCWSTR;
                pszProxyBypassW: LPCWSTR; dwFlags: DWORD): HINTERNET; stdcall;
    /// Sets up a callback function that WinHTTP can call as progress is made during an operation.
    SetStatusCallback: function(hInternet: HINTERNET; lpfnInternetCallback: TWinHttpStatusCallback;
                dwNotificationFlags: DWORD; dwReserved: NativeUInt): TWinHttpStatusCallback; stdcall;
    /// Specifies the initial target server of an HTTP request.
    Connect: function(hSession: HINTERNET; pswzServerName: LPCWSTR;
                nServerPort: INTERNET_PORT; dwReserved: DWORD): HINTERNET; stdcall;
    /// Creates an HTTP request handle.
    OpenRequest: function(hConnect: HINTERNET; pwszVerb: LPCWSTR; pwszObjectName: LPCWSTR;
                pwszVersion: LPCWSTR; pwszReferrer: LPCWSTR; ppwszAcceptTypes: PLPWSTR;
                dwFlags: DWORD): HINTERNET; stdcall;
    /// Closes a single HINTERNET handle.
    CloseHandle: function(hInternet: HINTERNET): BOOL; stdcall;
    /// Adds one or more HTTP request headers to the HTTP request handle.
    AddRequestHeaders: function(hRequest: HINTERNET; pwszHeaders: LPCWSTR;
                dwHeadersLength: DWORD; dwModifiers: DWORD): BOOL; stdcall;
    /// Sends the specified request to the HTTP server.
    SendRequest: function(hRequest: HINTERNET; lpszHeaders: LPCWSTR; dwHeadersLength: DWORD;
                lpOptional: Pointer; dwOptionalLength: DWORD; dwTotalLength: DWORD;
                dwContext: DWORD_PTR): BOOL; stdcall;
    /// Ends an HTTP request that is initiated by WinHttpSendRequest.
    ReceiveResponse: function(hRequest: HINTERNET; lpReserved: LPVOID): BOOL; stdcall;
    /// Retrieves header information associated with an HTTP request.
    QueryHeaders: function(hRequest: HINTERNET; dwInfoLevel: DWORD; pwszName: LPCWSTR;
                lpBuffer: Pointer; var lpdwBufferLength: DWORD; lpdwIndex: LPDWORD): BOOL; stdcall;
    /// Reads data from a handle opened by the WinHttpOpenRequest function.
    ReadData: function(hRequest: HINTERNET;
                lpBuffer: Pointer; dwNumberOfBytesToRead: DWORD;
                lpdwNumberOfBytesRead: LPDWORD): BOOL; stdcall;
    /// Sets the various time-outs that are involved with HTTP transactions.
    SetTimeouts: function(hInternet: HINTERNET; nResolveTimeout: Integer; nConnectTimeout: Integer;
                nSendTimeout: Integer; nReceiveTimeout: Integer): BOOL; stdcall;
    ...
    SetCredentials: function(hRequest: HINTERNET; AuthTargets: DWORD; AuthScheme: DWORD;
                pwszUserName: LPCWSTR; pwszPassword: LPCWSTR; pAuthParams: Pointer): BOOL; stdcall;
    ...
    CrackUrl: function(pwszUrl: LPCWSTR; dwUrlLength: DWORD; dwFlags: DWORD;
                var lpUrlComponents: TURLComponents): BOOL; stdcall;
    CreateUrl: function(var lpUrlComponents: TURLComponents; dwFlags: DWORD;
                pwszUrl: LPWSTR; var pdwUrlLength: DWORD): BOOL; stdcall;
    DetectAutoProxyConfigUrl: function(dwAutoDetectFlags: DWORD;
                ppwstrAutoConfigUrl: PLPWSTR): BOOL; stdcall;
    GetDefaultProxyConfiguration: function(var pProxyInfo: TWinHttpProxyInfo): BOOL; stdcall;
    SetDefaultProxyConfiguration: function(var pProxyInfo: TWinHttpProxyInfo): BOOL; stdcall;
    GetIEProxyConfigForCurrentUser: function(var pProxyConfig: TWinHttpCurrentUserIEProxyConfig): BOOL; stdcall;
    ...
    //IsHostInProxyBypassList: function(var pProxyInfo: TWinHttpProxyInfo;
    //          pwszHost: LPCWSTR; tScheme: TInternetScheme; nPort: TInternetPort;
    //          out pfIsInBypassList: BOOL): DWORD; stdcall;

    //QueryAuthParams: function(hRequest: HINTERNET; AuthScheme: DWORD;
    //            out pAuthParams: Pointer): BOOL; stdcall;
    QueryAuthSchemes: function(hRequest: HINTERNET; out lpdwSupportedSchemes: DWORD;
                out lpdwFirstScheme: DWORD; out pdwAuthTarget: DWORD): BOOL; stdcall;
    QueryDataAvailable: function(hRequest: HINTERNET; lpdwNumberOfBytesAvailable: LPDWORD): BOOL; stdcall;
    QueryOption: function(hInternet: HINTERNET; dwOption: DWORD; out lpBuffer;
...
type
  ...
  TWinHttpAPIs = (
    ...
    hCrackUrl,
    hCreateUrl,
    hDetectAutoProxyConfigUrl,
    hGetDefaultProxyConfiguration,
    hSetDefaultProxyConfiguration,
    hGetIEProxyConfigForCurrentUser,
    hGetProxyForUrl,
    //hIsHostInProxyBypassList,
    //hQueryAuthParams,
    hQueryAuthSchemes,
    hQueryDataAvailable,
    hQueryOption
...
var api: TWinHttpAPIs;
    P: PPointer;
    pCheckFun: function():BOOL; stdcall;
begin
  ...
  if WinHttpAPI.LibraryHandle=0 then
    raise EWinHttpAPI.CreateFmt('Unable to load library %s',[WinHttpDll]);

  //
  pCheckFun := GetProcAddress(WinHttpAPI.LibraryHandle, WinHttpNames[hCheckPlatform]);
  if not Assigned(pCheckFun) or not pCheckFun then
  begin
    FreeLibrary(WinHttpAPI.LibraryHandle);
    WinHttpAPI.LibraryHandle := 0;
    raise EWinHttpAPI.CreateFmt('Unsupported version %s',[WinHttpDll]);
  end;

  P := @@WinHttpAPI.CheckPlatform;
  ...
  begin
    ...
    begin
      FreeLibrary(WinHttpAPI.LibraryHandle);
      WinHttpAPI.LibraryHandle := 0;
      raise EWinHttpAPI.CreateFmt('Unable to find %s() in %s',[WinHttpNames[api], winhttpdll]);
    end;
    inc(P);
  end;
end;
